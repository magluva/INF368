...
def lenet_model(u1, u2, dim, n_classes, act):
    """
    ...
    """
    model = Sequential()
    model.add(Conv2D(filters=6, kernel_size=(3, 3), activation=act, input_shape=dim, kernel_regularizer=l2(0.01)))
    model.add(AveragePooling2D())
    model.add(Conv2D(filters=16, kernel_size=(3, 3), activation=act, kernel_regularizer=l2(0.01)))
    model.add(AveragePooling2D())
    model.add(Flatten())
    model.add(Dense(units=u1, activation=act, kernel_regularizer=l2(0.01)))
    model.add(Dense(units=u2, activation=act, kernel_regularizer=l2(0.01)))
...
def strat_k_fold(n_folds, X, y, u1, u2, dim, n_classes, n_epochs):
    ...
    for i, (train, val) in enumerate(skf.split(X_tr, y_tr)):
        ...
        # Compiling model.
        model = lenet_model(u1, u2, dim, n_classes, CV_actications[i])
...
def main():
    ...
    with open("res_dict_CV_K2_L2", 'wb') as f:
        pickle.dump(res, f)
